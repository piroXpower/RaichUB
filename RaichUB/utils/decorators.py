    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                raichureg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                raichureg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(raichureg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                raichureg = "^" + Config.SUDO_COMMAND_HAND_LER
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            text = "**RaichUB Error report**\n\n"
            link = "[here](https://t.me/RaichuOfficial)"
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            raichub.add_event_handler(func, MessageEdited(**args))
        raichub.add_event_handler(func, NewMessage(**args))
        try:
